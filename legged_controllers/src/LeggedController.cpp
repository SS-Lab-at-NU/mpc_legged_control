...
void LeggedController::update(const ros::Time& time, const ros::Duration& period) {
  // ROS_INFO_STREAM("[Debug]:Fun update");
  ...
  // ROS_INFO_STREAM("[Debug]:optimized State");
  // std::cout<<optimizedState<<std::endl;
  // ROS_INFO_STREAM("[Debug]:optimized Input");
  ...
  // ROS_INFO_STREAM("[Debug]:wbc output"<<x);
  // std::cout<<x<<std::endl;
  wbcTimer_.endTimer();
...
void LeggedController::updateStateEstimation(const ros::Time& time, const ros::Duration& period) {
  // ROS_INFO_STREAM("[Debug]:Fun updateStateEstimation");
  ...
  for (size_t i = 0; i < hybridJointHandles_.size(); ++i) {
    ...
    // std::cout << "joint position of " << i << ":" <<jointPos(i) << std::endl;
    jointVel(i) = hybridJointHandles_[i].getVelocity();
  }
  for (size_t i = 0; i < contacts.size(); ++i) {
    contactFlag[i] = contactHandles_[i].isContact();
    // std::cout << "Contace state of " << i << ":" << contactFlag[i] << std::endl;
  }
  for (size_t i = 0; i < 4; ++i) {
    quat.coeffs()(i) = imuSensorHandle_.getOrientation()[i];
    // std::cout << "quat " << i << ":" << imuSensorHandle_.getOrientation()[i] << std::endl;
  }
  for (size_t i = 0; i < 3; ++i) {
    angularVel(i) = imuSensorHandle_.getAngularVelocity()[i];
    linearAccel(i) = imuSensorHandle_.getLinearAcceleration()[i];
    // std::cout << "AngularVel " << i << ":" <<angularVel(i) << std::endl;
    // std::cout << "LinearAccel " << i << ":" <<linearAccel(i) << std::endl;
  }
...
void LeggedController::setupLeggedInterface(const std::string& taskFile, const std::string& urdfFile, const std::string& referenceFile,
                                            bool verbose) {
                                              ROS_INFO_STREAM("[Debug]:Fun setupLeggedInterface");
  leggedInterface_ = std::make_shared<LeggedInterface>(taskFile, urdfFile, referenceFile);
  leggedInterface_->setupOptimalControlProblem(taskFile, urdfFile, referenceFile, verbose);
}

void LeggedController::setupMpc() {
  // ROS_INFO_STREAM("[Debug]:Fun setupMpc");
...
void LeggedController::setupMrt() {
  // ROS_INFO_STREAM("[Debug]:Fun setupMrt");
...
void LeggedController::setupStateEstimate(const std::string& taskFile, bool verbose) {
  // ROS_INFO_STREAM("[Debug]:Fun setupStateEstimate");
